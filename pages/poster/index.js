...
Page({
  ...
  onLoad: function(options) {
    ...
    if (options.qrCode) {
      var avater = 'cardInfo.avater';
      var qrCode = 'cardInfo.qrCode';
      var Name = 'cardInfo.Name';
      this.setData({
        [avater]: options.avater,
  ...
  sharePosteCanvas: function(avaterSrc, codeSrc) {
    ...
    wx.createSelectorQuery().select('#canvas-container').boundingClientRect(function(rect) {
      ...
      //头像为正方形
      if (avaterSrc) {
        let avatarurl_width = width; //绘制的头像宽度
        let avatarurl_heigth = width; //绘制的头像高度
        let avatarurl_x = left; //绘制的头像在画布上的位置
      ...
      if (codeSrc) {
        ...
        ctx.setFontSize(10);
        ctx.setFillStyle('#000');
        //ctx.fillText("微信扫码或长按识别", left + 150, width + 150);
      }
    }).exec()
